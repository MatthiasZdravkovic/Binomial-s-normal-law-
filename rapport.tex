...
$$\frac{\partial \ln \mathcal{L}(\Sigma | z_i)}{\partial \Sigma}=\sum_{1}^{N}(2\pi\sqrt{\det\Sigma}\cdot\frac{\partial (\frac{1}{2\pi\sqrt{\det \Sigma}})}{\partial \Sigma})-\frac{1}{2}(z_i-\mu)(z_i-\mu)^{T}=0$$
$$\frac{\partial \ln \mathcal{L}(\Sigma | z_i)}{\partial \Sigma}=\sum_{1}^{N}(\sqrt{\det\Sigma}\cdot(-\frac{1}{2\det\Sigma \sqrt{\det \Sigma}})\cdot\det(\Sigma)(\Sigma^{-1})^{T})-\frac{1}{2}(z_i-\mu)(z_i-\mu)^{T}=0$$
$$\frac{\partial \ln \mathcal{L}(\Sigma | z_i)}{\partial \Sigma}=-\frac{1}{2}\sum_{1}^{N}\cdot(\Sigma^{-1})^{T}-\frac{1}{2}(z_i-\mu)(z_i-\mu)^{T}=0$$

Or on sait que $\Sigma$ est une matrice de rotation, donc $\Sigma^{T}=\Sigma^{-1}$.
$$\Leftrightarrow \frac{\partial \ln \mathcal{L}(\Sigma | z_i)}{\partial \Sigma}=-\frac{1}{2}\sum_{1}^{N}\Sigma-\frac{1}{2}(z_i-\mu)(z_i-\mu)^{T}=0$$
$$\Leftrightarrow \sum_{1}^{N}\Sigma=\sum_{1}^{N}(z_i-\mu)(z_i-\mu)^{T}$$
$$\Leftrightarrow (N-1)\cdot\Sigma=\sum_{1}^{N}(z_i-\mu)(z_i-\mu)^{T}$$
$$\Leftrightarrow \hat{\Sigma}=\frac{\sum_{1}^{N}(z_i-\mu)(z_i-\mu)^{T}}{N-1}$$
